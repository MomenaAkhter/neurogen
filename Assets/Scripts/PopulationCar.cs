...
namespace NeuroGen
{
    public class PopulationCar : PopulationProxy
    {
        ...
        public bool EveryoneIsDead => cars.FirstOrDefault(x => !x.isRunning) == null;
        ...
        {
            var cameraTarget = cars.OrderByDescending(x => x.Fitness).FirstOrDefault(x => x.gameObject.activeSelf);
        ...
        {
            base.InitPopl();
            cars = new List<Car>(Config.genomeCount);
        }

        protected override void InitGenomeProxyObj(GameObject genomeProxy)
        {
            var genomeCar = genomeProxy.GetComponent<Car>();
